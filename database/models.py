...
class Message(db.Model):
    ...
    status        = db.Column(db.Enum('PENDING', 'SENT', 'FAILED', 'RECEIVED', 'TIMEOUT'), default='PENDING')
...
class Campaign(db.Model):
    ...
    status       = db.Column(db.Enum('PENDING', 'RUNNING', 'DONE', 'FAILED', 'TIMEOUT'), default='PENDING')

    messages     = db.relationship('Message', backref='campaign', lazy=True)

class ATCommand(db.Model):
    __tablename__ = 'at_command'
...
# ─────────────────────────────────────────────────────────────────────────────
# Aliases for the standalone worker.py (so you can `from database.models import Outbox, Inbound`)
Outbox  = Message
Inbound = Inbox
